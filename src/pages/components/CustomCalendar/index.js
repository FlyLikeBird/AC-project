...
function CustomCalendar({ currentDate, data, onChangeDate, mode, energyInfo, onDispatch, theme }){
    ...
    function dateCellRender(value){
        ...
        let lessToday = value.toDate().getTime() <= today.getTime() ? true : false;
                        ...
                        <div className={style['content-head']}>{ value.date() } </div>             
                        {
                            ...
                            <div className={style['content-body']} style={{ opacity:lessToday ? '1' : '0' }}>
                                <div>
                                    ...
                                    <span className={style['text']} style={{ marginLeft:'4px' }}>{ lessToday ? (+cost).toFixed(1): '-- --'}</span>
                                    {/* <span className={style['unit']}>{ ' / ' + Math.round(quota) }</span> */}
                                    <span className={style['unit']}>{ lessToday ? '元' : ''}</span>
                                    {/* <span style={{ top:'3rem', position:'absolute', width:'14px', height:'18px', borderRadius:'4px', background:'#0c9dff', left:'-10px'  }}></span> */}
                                </div>
                                <div>
                                    {/* <span style={{ fontSize:'0.8rem', color:theme === 'dark' ? '#fff' : '#000' }}>{ info[energyInfo.type_code].text + '能耗 :' } </span> */}
                                    <span className={style['text']} style={{ marginLeft:'4px', fontWeight:lessToday ? 'bold' : 'normal' }}>{ lessToday ? (+energy).toFixed(1) : '-- --'}</span>
                                    <span className={style['unit']}>{ lessToday ? energyInfo.unit : ''}</span>
                                    {/* <span style={{ top:'5rem', position:'absolute', width:'14px', height:'18px', borderRadius:'4px', background:'#79e9ef', left:'-10px'  }}></span> */}
                                </div>
                            </div>
                            :
                            null
                ...
                <div className={style['content']} style={{ justifyContent:'space-around', padding:'1rem' }}>
                    ...
                    <div className={style['content-body']} style={{ opacity:lessMonth ? '1' : '0' }}>
                        <div style={{ marginBottom:'1rem' }}>
                            <div style={{ marginRight:'1rem', borderRadius:'12px', border:`1px solid ${info[energyInfo.type_code].color}`, padding:'4px 1rem', fontSize:'0.8rem', color:info[energyInfo.type_code].color }}>{ '成本 ' }</div>
                            <div className={style['text']} style={{ marginLeft:'4px' }}>{ (+cost).toFixed(1) }</div>
                            {/* <span className={style['unit']}>{ ' / 定额 : ' + quota }</span> */}
                            <span className={style['unit']}>元</span>
                        </div>  
                        <div>
                            <div style={{ marginRight:'1rem', borderRadius:'12px', border:`1px solid ${info[energyInfo.type_code].color}`, padding:'4px 1rem', fontSize:'0.8rem', color:info[energyInfo.type_code].color }}>{ '能耗 ' }</div>
                            <div className={style['text']} style={{ marginLeft:'4px' }}>{ (+energy).toFixed(1) }</div>
                    ...
                    value=>{
                        if ( onChangeDate && typeof onChangeDate === 'function' ) {
                            ...
                            if ( mode === 'year') {
                                // 月历模式下点击进入到某个月的日历
                                onChangeDate(value);
                                onDispatch({ type:'cost/setMode', payload:'month' });
                                onDispatch({ type:'cost/fetchCalendar'});
                            } 
                                // 日历模式下跳转不同月份
